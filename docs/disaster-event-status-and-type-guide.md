...
`startTime`、`endTime`、观测数据的 `observedAt` 和态势评估的 `assessmentTime` 均使用 ISO 8601 时间格式，并且必须显式使用北京时间偏移 `+08:00`。
...
- `+08:00` 表示东八区，也是请求允许使用的唯一偏移。
- `Z` 表示 UTC，不能用于本平台的 API 请求。
- 系统最多支持毫秒精度，即小数点后最多 3 位。

### 3.2 北京时间请求、输出与 UTC 存储

- 请求时间必须显式使用 `+08:00`。使用 `Z` 或其他时区偏移时返回 HTTP `422`，错误码为 `VALIDATION_ERROR`。
- API 响应中的事件时间、观测时间、评估时间、创建时间和错误时间统一转换为北京时间 `Asia/Shanghai`，显示为 `+08:00`。
- 应用日志时间统一使用北京时间。
- MySQL 与内部领域模型继续使用 UTC/`Instant` 保存和计算，不直接改写历史数据。
...
系统内部会把它转换为同一时刻的 `2026-08-21T00:00:00Z` 后保存，但响应仍显示为 `+08:00`。

以下时间包含微秒，不符合当前精度要求：

```text
2026-08-21T17:01:58.775123+08:00
```
...
| `422` | `VALIDATION_ERROR` | 请求时间未使用 `+08:00`、状态与开始/结束时间组合不合法、时间精度超过毫秒或观测指标不合法 |
